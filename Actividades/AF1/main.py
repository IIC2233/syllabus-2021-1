...
from consultas import (descartar_platos, platos_por_categoria,
                       ordenar_plato, resumen_orden)
...
def menu_descartar(descartados, ingredientes):
    ...
    while respuesta != 0:
        ...
        for i in range(0, len(nombre_ing), 3):
            ing = nombre_ing[i]
            seleccion = f"[{i + 1}]({'X' if ing in descartados else ' '}) {ing}"
            print_ing += seleccion.ljust(25)
            if i + 1 < len(ingredientes):
                ing = nombre_ing[i + 1]
                seleccion = f"[{i + 2}]({'X' if ing in descartados else ' '}) {ing}"
                print_ing += seleccion.ljust(25)
            if i + 2 < len(ingredientes):
                ing = nombre_ing[i + 2]
                seleccion = f"[{i + 3}]({'X' if ing in descartados else ' '}) {ing}"
                print_ing += seleccion.ljust(25)
            print_ing += "\n"
        print(print_ing + "\n[0] Continuar Orden\n")
...
def menu_agregar(platos, ingredientes, platos_ordenados):
    ...
    if respuesta_2 != 0:
        ...
        if respuesta_3 != 0:
            ...
            if ordenar_plato(plato, ingredientes):
                platos_ordenados.append(plato)
                print("¡Plato ordenado correctamente!")
            else:
                print("No se ha podido ordenar este plato :(")
            input("\n* Presione cualquier tecla para continuar *")
...
def main(platos, ingredientes):
    ...
    while respuesta != 0:
        ...
        print("\n¡Hola bienvenido a Progratouille!")
        ...
        if respuesta == 1:
            menu_descartar(ingredientes_descartados, ingredientes)
            if ingredientes_descartados:
                platos_filtrados = descartar_platos(ingredientes_descartados, platos)
